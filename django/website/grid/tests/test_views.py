...
class FunctionalGridTest(GridTestCase):
    ...
    def test_edit_feature_view(self):
        url = reverse('edit_feature', kwargs={'id': self.feature1.id})
        response = self.client.get(url)
    ...
    def test_delete_feature_view(self):
        ...
        self.assertTrue(self.client.login(username='user',
                                          password=STOCK_PASSWORD))
        url = reverse('delete_feature', kwargs={'id': self.feature1.id})
        self.client.get(url)
        ...
        self.assertTrue(self.client.login(username='cleaner',
                                          password='cleaner'))
        self.client.get(url)
        self.assertEqual(Feature.objects.count(), count - 1)

    def test_edit_element_view(self):
        url = reverse('edit_element',
        ...
        url = reverse('edit_element',
                      kwargs={'feature_id': self.feature1.id,
                              'package_id': self.gridpackage4.id})
        response = self.client.get(url)
    ...
    def test_delete_gridpackage_view(self):
        ...
        self.assertTrue(self.client.login(username='user',
                                          password=STOCK_PASSWORD))
        url = reverse('delete_grid_package',
                      kwargs={'id': self.gridpackage1.id})
        self.client.get(url)
...
class RegressionGridTest(GridTestCase):
    def setUp(self):
        super(RegressionGridTest, self).setUp()
        settings.RESTRICT_GRID_EDITORS = False
    ...
    def test_edit_element_view_for_nonexistent_elements(self):
        ...
        """
        feature = self.feature1  # From tests/data.py
        gp = self.gridpackage1
        element, created = Element.objects.get_or_create(feature=feature,
                                                         grid_package=gp)
        ...
        url = reverse('edit_element', kwargs={'feature_id': feature.id,
                                              'package_id': gp.id})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'grid/edit_element.html')
...
class GridPermissionTest(GridTestCase):
    def setUp(self):
        super(GridPermissionTest, self).setUp()
...
class GridPackagePermissionTest(GridTestCase):
    def setUp(self):
        super(GridPackagePermissionTest, self).setUp()
        settings.RESTRICT_GRID_EDITORS = True
...
class GridFeaturePermissionTest(GridTestCase):
    def setUp(self):
        super(GridFeaturePermissionTest, self).setUp()
        settings.RESTRICT_GRID_EDITORS = True
        id = self.feature1.id
        self.test_add_url = reverse('add_feature',
                                    kwargs={'grid_slug': 'testing'})
        self.test_edit_url = reverse('edit_feature', kwargs={'id': id})
        self.test_delete_url = reverse('delete_feature',  kwargs={'id': id})
        self.login = self.client.login(username='user',
                                       password=STOCK_PASSWORD)
        self.user = User.objects.get(username='user')
